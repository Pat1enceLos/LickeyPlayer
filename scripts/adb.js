...
(function () {
  ...
  Adb.WebUSB.Transport.prototype.find = function (filter) {
    for (const i in this.device.configurations) {
      const conf = this.device.configurations[i];
      for (const j in conf.interfaces) {
        const intf = conf.interfaces[j];
        for (const k in intf.alternates) {
          const alt = intf.alternates[k];
    ...
    return this.getDevice({
      ...
      .then(fastboot => fastboot.send('getvar:max-download-size')
        .then(() => fastboot.receive()
          .then((response) => {
            const cmd = decode_cmd(response.getUint32(0, true));
  ...
  Adb.Message.send = function (device, message) {
    ...
    const cmd = encode_cmd(message.cmd);
  ...
  Adb.Message.receive = function (device) {
    return device.receive(24) // Adb.Opt.use_checksum ? 24 : 20)
      .then((response) => {
        ...
        if (len == 0) {
          const message = new Adb.Message(cmd, arg0, arg1);
          if (Adb.Opt.debug) { console.log(message); }
          return message;
        }
        ...
        return device.receive(len)
          .then((data) => {
            ...
            const message = new Adb.Message(cmd, arg0, arg1, data);
            if (Adb.Opt.debug) { console.log(message); }
            return message;
          });
      });
  ...
  Adb.SyncFrame.send = function (stream, frame) {
    ...
    const cmd = encode_cmd(frame.cmd);
    ...
    return stream.receive()
      .then((response) => {
        if (response.cmd == 'WRTE') {
          const cmd = decode_cmd(response.data.getUint32(0, true));

          if (cmd == 'OKAY' || cmd == 'DATA' || cmd == 'DONE' || cmd == 'FAIL') {
            const len = response.data.getUint32(4, true);
            const data = new DataView(response.data.buffer.slice(8));

            if (len == 0 || data.byteLength >= len) {
              const frame = new Adb.SyncFrame(cmd, len, data);
              if (Adb.Opt.debug) { console.log(frame); }
              return frame;
            }
            ...
            return stream.send('OKAY')
              ...
              .then((response) => {
                if (response.data == null) {
                  const frame = new Adb.SyncFrame(cmd);
                  if (Adb.Opt.debug) { console.log(frame); }
                  return frame;
                }

                const cmd2 = decode_cmd(response.data.getUint32(0, true));
                ...
                if (cmd2 == 'OKAY' || cmd2 == 'DATA' || cmd2 == 'DONE' || cmd2 == 'FAIL') {
                  ...
                  if (len == 0 || data.byteLength >= len) {
                    const frame = new Adb.SyncFrame(cmd2, len, data);
                    if (Adb.Opt.debug) { console.log(frame); }
                    return frame;
                  }
                }
                ...
                const frame = new Adb.SyncFrame(cmd, len, response.data);
                if (Adb.Opt.debug) { console.log(frame); }
                return frame;
              });
          }
        ...
        if (response.cmd == 'OKAY') {
          const frame = new Adb.SyncFrame('OKAY');
          if (Adb.Opt.debug) { console.log(frame); }
          return frame;
        }
  ...
  Adb.Stream.open = function (device, service) {
    const local_id = next_id++;
    let remote_id = 0;

    const m = new Adb.Message('OPEN', local_id, remote_id, `${service}\0`);
    return m.send_receive(device)
      .then(function do_response(response) {
        if (response.arg1 != local_id) {
          return Adb.Message.receive(device)
            .then(do_response);
        }
  ...
  Adb.Stream.prototype.send = function (cmd, data = null) {
    const m = new Adb.Message(cmd, this.local_id, this.remote_id, data);
  ...
  Adb.Stream.prototype.stat = function (filename) {
    const frame = new Adb.SyncFrame('STAT', filename.length);
    return frame.send_receive(this)
      ...
      .then(check_ok(`STAT failed on ${filename}`))
      .then(response => this.receive()
        .then(response =>
          this.send('OKAY')
            .then(() =>
              response.data)))
      .then((response) => {
        const id = decode_cmd(response.getUint32(0, true));
        const mode = response.getUint32(4, true);
        const size = response.getUint32(8, true);
        const time = response.getUint32(12, true);
  ...
  Adb.Stream.prototype.pull = function (filename) {
    const frame = new Adb.SyncFrame('RECV', filename.length);
    return frame.send_receive(this)
  ...
  Adb.Stream.prototype.push_start = function (filename, mode) {
    const mode_str = mode.toString(10);
    const encoder = new TextEncoder();

    const frame = new Adb.SyncFrame('SEND', filename.length + 1 + mode_str.length);
    return frame.send_receive(this)
      .then(check_ok(`PUSH failed on ${filename}`))
      .then(response => this.send('WRTE', encoder.encode(filename)))
      .then(() => Adb.SyncFrame.receive(this))
      .then(check_ok(`PUSH failed on ${filename}`))
      .then(response => this.send('WRTE', encoder.encode(`,${mode_str}`)))
      .then(() => Adb.SyncFrame.receive(this))
      .then(check_ok(`PUSH failed on ${filename}`));
  };
  ...
  Adb.Stream.prototype.push_data = function (data) {
    ...
    const frame = new Adb.SyncFrame('DATA', data.byteLength);
    return frame.send_receive(this)
      .then(check_ok('PUSH failed'))
      .then(response => this.send('WRTE', data))
      .then(() => Adb.SyncFrame.receive(this))
      .then(check_ok('PUSH failed'));
  };

  Adb.Stream.prototype.push_done = function () {
    const frame = new Adb.SyncFrame('DONE', Math.round(Date.now() / 1000));
    return frame.send_receive(this)
      .then(check_ok('PUSH failed'))
      .then(response => Adb.SyncFrame.receive(this))
      .then(check_ok('PUSH failed'))
      .then(response => this.send('OKAY'));
  };
  ...
  Adb.Stream.prototype.push = function (file, filename, mode, on_progress = null) {
    ...
    const old_debug = Adb.Opt.debug;
    const old_dump = Adb.Opt.dump;
    Adb.Opt.debug = false;
    Adb.Opt.dump = false;
    ...
    return read_blob(file)
      .then(data =>
        this.push_start(filename, mode)
          .then(() => {
            let seq = Promise.resolve();
            let rem = file.size;
            const max = Math.min(0x10000, this.device.max_payload);
            while (rem > 0) {
  ...
  Adb.Stream.prototype.quit = function () {
    const frame = new Adb.SyncFrame('QUIT');
    return frame.send_receive(this)
  ...
  function generate_key() {
    const extractable = Adb.Opt.dump;
  ...
  function do_auth(adb, keys, key_idx, token, do_auth_response, auth_user_notify) {
    ...
    if (key_idx < keys.length) {
      const slot = keys.length - key_idx - 1;
      const key = keys[slot];
      let seq = Promise.resolve();
      ...
      return seq.then(() => crypto.subtle.sign({ name: 'RSASSA-PKCS1-v1_5' }, key.privateKey, token))
        .then((signed) => {
          ...
          const m = new Adb.Message('AUTH', AUTH_SIGNATURE, 0, signed);
    ...
    return seq.then(key => crypto.subtle.exportKey('spki', key.publicKey)
      .then((pubkey) => {
        const m = new Adb.Message('AUTH', AUTH_RSAPUBLICKEY, 0, `${toB64(pubkey)}\0`);
        return m.send(adb);
      })
      .then(() => {
  ...
  function init_db() {
    const req = window.indexedDB.open('WebADB', 1);
